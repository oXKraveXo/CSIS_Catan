...
public class CatanModel {
	...
	public CatanModel(String [] input) {
		for (int i=0; i<users.length; i++) {
			users[i] = new User(" ", " ", i, 5, 4, 15, usersCards, 0, resources );
		}
		int counter= 0;
		for (int i =0; i< users.length; i++) {	
			users[i].setName(input[counter]); 
			counter++;
			}
		for (int i =0; i <users.length; i++) {
			users[i].setColor(input[counter]); 
			counter++;
		}
		}
	...
	public String toString() {
		return "CatanModel [users=" + Arrays.toString(users) + ", usersCards=" + Arrays.toString(usersCards) + ", deck="
				+ deck + ", sc=" + sc + ", resources=" + Arrays.toString(resources) + ", WOOD=" + WOOD + ", SHEEP="
				+ SHEEP + ", WHEAT=" + WHEAT + ", BRICK=" + BRICK + ", STONE=" + STONE + ", winAmount=" + winAmount
				+ ", whoseTurn=" + whoseTurn + "]";
	}
